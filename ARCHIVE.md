...
| `0x5b` | `SyncPartyMemberPositions` | Copies each party's visible leader's object position (render offsets `$02-$07` + tile/collision `$13-$14`) onto every member of parties 1-3. Fixes the field party-swap "teleport" bug: when a party's leader is changed mid-field and the party walks away, a demoted follower keeps a stale object position; promoting it again (e.g. via the two-party swap menu) redraws the party at the stale spot. Called from the JOIN/SWAP party-interaction scripts (`event/ruination.py` `create_party_interaction_scripts`) right before the party-select menu. No-op for already-synced members. |
