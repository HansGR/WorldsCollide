...
def modify_thamasa_inn_ruination(rom, dialogs):
    """
    Modifies the Thamasa inn for ruination mode.

    In ruination mode:
    - If player has Strago AND burning house not done: charge 1 GP (to access the event)
    - Otherwise: charge normal inn price (200 GP * multiplier)
    ...
    # Dialog IDs
    THAMASA_INN_DIALOG = 0x0790  # Was "strangers" dialog, repurposed as normal price

    # Event addresses
    INN_CHECK_START = 0xbd73f      # Start of inn event checks (8 bytes available)
    NORMAL_PRICE_PATH = 0xbd769   # "Stranger" path, repurposed as normal price
    NORMAL_PRICE_GP = 0xbd775     # GP cost location for normal price path

    # Build the new check: If DEFEATED_FLAME_EATER is set, branch to normal price path
    # Format: C0 [event_bit | 0x8000 as 2 bytes LE] [destination - EVENT_CODE_START as 3 bytes LE]
    defeated_flame_eater = event_bit.DEFEATED_FLAME_EATER
    event_bit_bytes = (defeated_flame_eater | 0x8000).to_bytes(2, 'little')
    destination_bytes = (NORMAL_PRICE_PATH - EVENT_CODE_START).to_bytes(3, 'little')

    # Instruction: BranchIfEventBitSet(DEFEATED_FLAME_EATER, NORMAL_PRICE_PATH)
    # Opcode 0xC0 + 2 bytes event bit + 3 bytes destination = 6 bytes
    branch_instruction = bytes([0xC0]) + event_bit_bytes + destination_bytes

    # Fill remaining space with NOPs (opcode 0xFD)
    # Available space is 8 bytes (0xbd73f to 0xbd746), instruction is 6 bytes
    padding = bytes([0xFD, 0xFD])  # 2 NOPs

    # Write the new check at the start of the inn event
    rom.set_bytes(INN_CHECK_START, branch_instruction + padding)

    # Update the "normal price" path dialog and cost
    # Change dialog from "You're strangers... 1500 GP" to normal inn dialog
    dialogs.set_text(THAMASA_INN_DIALOG,
                     f"{new_price} GP per night.<line>Stay a while?<line>"
                     f"<choice> Yes<line><choice> No<end>")

    # Update the GP cost for normal price path
    rom.set_bytes(NORMAL_PRICE_GP, new_price.to_bytes(2, 'little'))

    if rom.args.debug:
        print(f"Modified Thamasa inn for ruination mode:")
        print(f"  - Burning house not done: 1 GP")
        print(f"  - Burning house done: {new_price} GP")
        print(f"  - Patched event check at {INN_CHECK_START:#x}")
