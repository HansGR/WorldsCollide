...
- **args/ruin_preprocessor.py** — `-ruin` flagset expansion: `RUIN_DEFAULT_FLAGS` (the shared base, incl. `-nfh`/`-oss`) + `RUIN_STANDARD_EXTRAS` (bare `-ruin` roguelike difficulty: `-pd`, `-sfd 3`, `-rls 53,42,51`). `-ruin easy` = base only; `-ruin custom` = no injection; `-no <flag>` removes any default. Default-flag values must be space-free (spell ids, not names) or the logged flag string won't round-trip.
