...
- **args/ruin_preprocessor.py** — `-ruin` flagset expansion: `RUIN_DEFAULT_FLAGS` (the base set, expanded by a bare `-ruin`; incl. `-nfh`/`-oss`/`-rce 6.9`) + `RUIN_HARD_EXTRAS` (`-ruin hard` roguelike difficulty: `-pd`, `-sfd 3`, `-rls 53,42,50,51`, `-nosaves lite`). `-ruin custom` = no injection; `-no <flag>` removes any default. Default-flag values must be space-free (spell ids, not names) or the logged flag string won't round-trip. KT unlock character/esper counts come from `-rce CC.EE` (or `cc.cc.ee.ee` ranges; default 6.9, min 3.0), processed in `args/doors.py` → `args.ruin_characters_required`/`ruin_espers_required`. Objectives are otherwise mode-independent, with one exception: `args/objectives.py` drops Kefka's Tower results (`KT_RESULT_IDS = {1, 2, 3}`) in ruination — they do nothing there and can softlock — which shifts any later objectives' letters/`OBJECTIVE` bits down a slot.
