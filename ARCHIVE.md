...
* `web/borders.js` — auto-generated by `scripts/build_borders.py`.
  Four hand-curated border presets extracted from the bottom 4×3 tile
  region of vanilla W1 / W2 / W4 / W6 and reindexed to
  `{0: transparent, 1: highlight, 7: outline}`.  Plus a `plain`
  (all-transparent) preset.  The reindex lets the border compose with
  any user texture: the texture quantizer sorts its 7 colors
  light→dark into palette slots 1..7, so the border's
  `highlight`/`outline` lands on the extremes of whatever palette
  came out of the upload.
