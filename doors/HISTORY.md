...
| 23 | 2026-09-04 | — | **`-rrt` (require towns)**: `finalize_branch` wires a shop town (`SHOP_TOWN_ROOMS`) into every branch — an unplaced member town first, else one pulled from the unclaimed towns (door-neutral first), by door or by a trap into the town's pit; three chances (after the warp rescue, before the terminus door, before the dead-end doors) and a re-roll if a branch still closes without one. Part of the `-ruin easy` preset. Off by default; bare `-ruin` seeds byte-identical. |
| 24 | 2026-09-26 | — | **Walk reachability rules + short restarts**: Rules G/H (`plan/prune.py`) keep every cluster reachable from home and able to get back, during the walk and on the finished map (found by a retrospective check: e.g. 15/200 `-dre` Serpent Trench layouts sealed the player in a current loop). The walk budget is spent in 2,000-connection restarts (`-drx` seed that failed after 5 x 50,000 now solves in seconds). Separately, KT lane `verify()` was brought back in sync with the room data (crossings two-way; derived from `room_data`): vanilla-tower fallbacks 18/200 -> 0/200. |
