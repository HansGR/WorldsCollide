...
* `web/borders.js` — auto-generated by `scripts/build_borders.py`.
  ...
  `plain` preset.  Each vanilla pixel is reindexed to its **luminosity
  rank** within that window's source palette: vanilla 0 stays
  transparent, the lightest vanilla color → output index 1, the next →
  2, … the darkest → 7.  Preserving the full 7-level rank keeps every
  anti-aliased corner, bolt highlight, and gauge shading that the
  vanilla art was using — collapsing to a two-color `{highlight,
  outline}` silhouette (the first cut) made the textured borders
  (W2/W4/W6) read as noise.  The reindex composes with any user
  texture because the texture quantizer also sorts its 7 colors
  light→dark into the same slots, so a border pixel at "rank 3" lands
  on whatever the user's 3rd-lightest texture color is.
