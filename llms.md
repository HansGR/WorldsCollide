...
## 9. Offline Tooling: `ruination_spoiler/`

`ruination_spoiler/` is a standalone, offline analysis package (no ROM needed) that parses a Ruination spoiler log and rebuilds the three branch door-maps, reproducing the figure `Ruination.generate_map_image` draws in-process. Use it to inspect a finished seed for mapping problems (e.g. a suspected softlock).

- **Run**: `python -m ruination_spoiler <spoiler.txt> [-o map.png] [--no-image]`. The log must be generated with `-debug-verbose`; reconstruction reads the per-branch `Making connection` / `Selected ... (room R)` trace from the "Debug Verbose Diagnostics" section.
- **Door -> room disambiguation**: `data/rooms.py` lists some doors in multiple candidate rooms; only the room in `RUIN_ROOM_SETS` (loaded from `event/ruination.py`) is the one ruination uses. `ruin_data.py` builds the door->room index restricted to that set, and the reconstructor prefers room names from the spoiler's own reward/terminus paths. The three Narshe-School hub doors (393/394/395) share the static `ruin_hub` room and are split one-per-branch.
- **Import isolation**: the package imports `data.rooms.room_data` (a leaf, ROM-free), but reads `RUIN_ROOM_SETS`/`ROOM_REWARD`/etc. literals out of `event/ruination.py` source with `ast` instead of importing it (that module pulls in the full event stack and triggers argparse at import). `parser.py`/`analyze.py` are standard-library only; `render.py` is the only part needing `networkx`+`matplotlib`.
- **Regression test/fixture**: `tests/test_ruination_spoiler.py` + `tests/fixtures/ruin_sample_spoiler.txt`. Run `python -m unittest tests.test_ruination_spoiler`. If you change the reconstruction logic, re-run it.
