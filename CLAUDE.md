...
- **data/doors.py** — `Doors`: thin Data-phase shell around the `doors/` planner (`mod()` calls `plan_for_args`, owns `self.plan`/`self.map`); also `print()` (spoiler section) and the `-debug_dest` BFS route printer. The per-mode table adjustments (town split exits, ruination forcing exclusions, shuffle-room door edits) are per-plan views built in `doors/plan` and carried on the DoorPlan (`plan.shared_exits`, `plan.forcing`) — the shared data tables are never mutated; the only per-build scratch is exit_data's dc-override + the event_exit_info runtime address column, reset in `Doors.__init__`. `verbose` is a property of `verbose.is_enabled()` — don't add an instance attribute.
...
- **`-rrt`** (`--ruination-require-towns`, part of `-ruin easy`) — `finalize_branch` wires a shop town (`data/ruin_constants.SHOP_TOWN_ROOMS`: the nine towns with shops; Tzen is the only single-door one) into every branch's hub region: an unplaced member town first, else one pulled from the towns no branch holds (`_pull_town`, single-room areas first, shops registered); rescue runs right after the warp rescue, again before step 4 once hub doors exist, and a branch closing without one raises `RuinPlanError` (re-roll). Off by default; without it every draw is unchanged (bare `-ruin` seeds byte-identical).
...
- **Race builds** — `feature/race-obfuscation` (not merged) adds `-race`: spoiler tables relocated + masked + decoyed, check rewards slot-indirected. Its docs live on that branch (`RACE_OBFUSCATION_OVERVIEW.md` first). Field opcodes `$E6`/`$FC` (chosen as free on both branches; the reward dialog rides vanilla `$4B` with operand bit 13), handlers in bank F0 behind 20 bytes of C0. See ARCHIVE.md "Race ROM Obfuscation".
